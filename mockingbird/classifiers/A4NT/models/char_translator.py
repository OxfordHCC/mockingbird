...
from classifiers.A4NT.models.model_utils import packed_mean, packed_add
...
def gumbel_softmax_sample(x, tau=0.2, hard=False):
    ...
    if hard:
        ...
        one_hot = Variable(ysft.data.new(ysft.size()).zero_().scatter_(1, max_idx.data,
                                                                       ysft.data.new(max_idx.size()).fill_(
                                                                           1.)) - ysft.data, requires_grad=False)
...
class CharTranslator(nn.Module):
    def __init__(self, params, encoder_only=False):
        ...
        if not self.no_encoder:
            if self.en_residual:
                ...
                self.enc_rec_layers = nn.ModuleList([nn.LSTM(inp_sizes[i], self.enc_hidden_size, 1) for i in
                                                     range(self.enc_num_rec_layers)])
            else:
        ...
        if not encoder_only:
            ...
            if self.en_residual:
                self.dec_rec_layers = nn.ModuleList([nn.LSTM(input_layer_size if i == 0 else self.dec_hidden_size,
                                                             self.dec_hidden_size, 1) for i in
                                                     range(self.dec_num_rec_layers)])
            else:
                self.dec_rec_layers = nn.LSTM(input_layer_size, self.dec_hidden_size, self.dec_num_rec_layers)

            # Output decoder layer
            self.decoder_W = nn.Parameter(torch.zeros([self.dec_hidden_size, self.vocab_size]), True)
            ...
            if self.split_gen:
                if self.en_residual:
                    self.dec_rec_layers_2 = nn.ModuleList([nn.LSTM(input_layer_size if i == 0 else self.dec_hidden_size,
                                                                   self.dec_hidden_size, 1) for i in
                                                           range(self.dec_num_rec_layers)])
                else:
        ...
        # self.cuda()
        # Create a dummy zero hidden state so it can be passed to lstm
        self.zero_hidden_bsz = params['batch_size']
        # Create a dummy zero hidden state so it can be passed to lstm
        self.zero_hidden = self.init_hidden(params['batch_size'])
        # Create a dummy zero hidden state so it can be passed to lstm
        self.zero_hidden_dec = self.init_hidden_dec(params['batch_size'])
    ...
    def init_weights(self):
        ...
        if self.en_residual:
            if not self.no_encoder:
                for i in range(self.enc_num_rec_layers):
                    self.enc_rec_layers[i].bias_ih_l0.data.index_fill_(0,
                                                                       torch.arange(enc_h_sz + 1, enc_h_sz * 2).long(),
                                                                       1.)
                    self.enc_rec_layers[i].bias_hh_l0.data.index_fill_(0,
                                                                       torch.arange(enc_h_sz + 1, enc_h_sz * 2).long(),
                                                                       1.)
            if not self.encoder_only:
                for i in range(self.dec_num_rec_layers):
                    self.dec_rec_layers[i].bias_ih_l0.data.index_fill_(0,
                                                                       torch.arange(dec_h_sz + 1, dec_h_sz * 2).long(),
                                                                       1.)
                    self.dec_rec_layers[i].bias_hh_l0.data.index_fill_(0,
                                                                       torch.arange(dec_h_sz + 1, dec_h_sz * 2).long(),
                                                                       1.)
                    if self.split_gen:
                        self.dec_rec_layers_2[i].bias_ih_l0.data.index_fill_(0, torch.arange(dec_h_sz + 1,
                                                                                             dec_h_sz * 2).long(), 1.)
                        self.dec_rec_layers_2[i].bias_hh_l0.data.index_fill_(0, torch.arange(dec_h_sz + 1,
                                                                                             dec_h_sz * 2).long(), 1.)
        else:
            if not self.no_encoder:
                for i in range(self.enc_num_rec_layers):
                    getattr(self.enc_rec_layers, 'bias_ih_l' + str(i)).data.index_fill_(0, torch.arange(enc_h_sz + 1,
                                                                                                        enc_h_sz * 2).long(),
                                                                                        1.)
                    getattr(self.enc_rec_layers, 'bias_hh_l' + str(i)).data.index_fill_(0, torch.arange(enc_h_sz + 1,
                                                                                                        enc_h_sz * 2).long(),
                                                                                        1.)
            if not self.encoder_only:
                for i in range(self.dec_num_rec_layers):
                    getattr(self.dec_rec_layers, 'bias_ih_l' + str(i)).data.index_fill_(0, torch.arange(dec_h_sz + 1,
                                                                                                        dec_h_sz * 2).long(),
                                                                                        1.)
                    getattr(self.dec_rec_layers, 'bias_hh_l' + str(i)).data.index_fill_(0, torch.arange(dec_h_sz + 1,
                                                                                                        dec_h_sz * 2).long(),
                                                                                        1.)
                    if self.split_gen:
                        getattr(self.dec_rec_layers_2, 'bias_ih_l' + str(i)).data.index_fill_(0,
                                                                                              torch.arange(dec_h_sz + 1,
                                                                                                           dec_h_sz * 2).long(),
                                                                                              1.)
                        getattr(self.dec_rec_layers_2, 'bias_hh_l' + str(i)).data.index_fill_(0,
                                                                                              torch.arange(dec_h_sz + 1,
                                                                                                           dec_h_sz * 2).long(),
                                                                                              1.)
    ...
    def _my_recurrent_layer(self, packed, h_prev=None, rec_func=None, n_layers=1.):
        if self.en_residual:
            p_out = packed
            hid_all = []
            rec_func = self.rec_layers if rec_func is None else rec_func
    ...
    def forward_mltrain(self, inp, lengths_inp, targ, lengths_targ, h_prev=None, compute_softmax=False, auths=None,
                        adv_inp=False, sort_enc=None, adv_targ=False):
        ...
        if not self.no_encoder:
            ...
            h_prev_enc = h_prev if h_prev is not None or b_sz != self.zero_hidden_bsz else self.zero_hidden
            enc_rnn_out, enc_hidden = self._my_recurrent_layer(packed, h_prev=h_prev_enc, rec_func=self.enc_rec_layers,
                                                               n_layers=self.enc_num_rec_layers)
            ...
            if self.pad_auth_vec:
                ...
                ctxt_sorted = torch.cat([ctxt_sorted, self.auth_emb(Variable(auths))], dim=-1)

            if self.enc_noise:
                ctxt_sorted = ctxt_sorted + Variable(torch.cuda.FloatTensor(ctxt_sorted.size()).normal_() / 20.,
                                                     requires_grad=False)
            else:
    ...
    def forward_encode(self, x, lengths_inp, h_prev=None, adv_inp=False):
        ...
        if self.encoder_mean_vec:
            ctxt = torch.cat([packed_mean(enc_rnn_out, dim=0), enc_hidden[0][-1]], dim=-1)
        else:
            ctxt = enc_hidden[0][-1]
        return ctxt

    def forward_advers_gen(self, x, lengths_inp, h_prev=None, n_max=100, end_c=-1, soft_samples=False, temp=0.1,
                           auths=None, adv_inp=False, n_samples=1):
        ...
        if self.pad_auth_vec:
            ...
            ctxt = torch.cat([ctxt, self.auth_emb(Variable(auths))], dim=-1)
        ...
        # Decode the output sequence using encoder state
        dec_rec_func = self.dec_rec_layers if not self.split_gen or auths[0] == 0 else self.dec_rec_layers_2
        dec_rnn_out, dec_hidden = self._my_recurrent_layer(dec_inp.view(1, dec_bsz, -1), h_prev=h_prev_dec,
                                                           rec_func=dec_rec_func, n_layers=self.dec_num_rec_layers)
        ...
        for i in range(n_max):
            ...
            if prev_done.all():
                ...
                p_rnn, dec_hidden = self._my_recurrent_layer(dec_inp, h_prev=dec_hidden, rec_func=dec_rec_func,
                                                             n_layers=self.dec_num_rec_layers)
    ...
    def forward_gen(self, x, h_prev=None, auths=None, n_max=100, end_c=-1, soft_samples=False, temp=0.1):
        ...
        # Encode the sequence of input characters
        enc_rnn_out, enc_hidden = self._my_recurrent_layer(packed, h_prev, rec_func=self.enc_rec_layers,
                                                           n_layers=self.enc_num_rec_layers)
        ...
        for i in range(n_max):
            ...
            if (pred_c == end_c).data[0]:
                ...
                p_rnn, dec_hidden = self._my_recurrent_layer(dec_inp, dec_hidden, rec_func=dec_rec_func,
                                                             n_layers=self.dec_num_rec_layers)
                p_rnn = p_rnn[-1]
