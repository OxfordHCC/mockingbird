...
from utils.utils import eval_translator, eval_classify
...
def disp_gen_samples(modelGen, modelEval, dp, misc, maxlen=100, n_disp=5, atoms='char', append_tensor=None):
    ...
    print
    '----------------------Visualising Some Generated Samples-----------------------------------------\n'
    for i in xrange(len(lens)):
        print
        '%d Inp : %6s --> %s' % (
        i, misc['ix_to_auth'][auths[i]], jc.join([ix_to_char[c] for c in inps.numpy()[1:, i] if c in ix_to_char]))
        print
        '  Out : %6s --> %s' % (misc['ix_to_auth'][1 - auths[i]],
                                jc.join([ix_to_char[c[i]] for c in char_outs[:gen_lens[i]] if c[i] in ix_to_char]))
        # print '%d Inp : %s --> %s' % (0, misc['ix_to_auth'][auths[0]], ' '.join([ix_to_char[c] for c in inps.numpy()[1:, 0] if c in ix_to_char]))
        print
        '  Rev : %6s --> %s\n' % (misc['ix_to_auth'][auths[i]], jc.join(
            [ix_to_char[c[i]] for c in rev_char_outs[:rev_lens[i]] if c[i] in ix_to_char]))
    print
    '\n-------------------------------------------------------------------------------------------------'
    modelGen.train()
    modelEval.train()
...
def adv_forward_pass(modelGen, modelEval, inps, lens, end_c=0, backprop_for='all', maxlen=100, auths=None,
    ...
    if cycle_compute and backprop_for != 'eval':
        ...
        if cycle_loss_type != 'ml':
            ...
            rev_gen_samples_orig_order = GradFilter(2)(
                rev_gen_samples_orig_order) if GradFilterLayer else rev_gen_samples_orig_order
            ...
            samples_out = (
            gen_samples_tensor, gen_lens, char_outs, rev_gen_samples_orig_order, rev_gen_lens, rev_char_outs)
        else:
...
def main(params):
    ...
    if params['resume'] == None and params['loadgen'] == None:
        ...
        if params['loadeval'] or params['resume']:
            ...
            model_eval_state = saved_eval_model['state_dict_eval'] if params[
                                                                          'loadeval'] == None else saved_eval_model[
                'state_dict']
    ...
    if params['language_loss']:
        ...
        for lmix, lm in enumerate(params['language_model']):
            ...
            if lang_model_cp['char_to_ix'] != char_to_ix:
                ...
                ix_to_oix = {ix: orig_chartoix[c] if c in orig_chartoix else orig_chartoix['UNK'] for c, ix in
                             char_to_ix.items()}
                mapVocabToLangModel[lmix] = Variable(
                    torch.cuda.FloatTensor(len(ix_to_oix) + 1, len(orig_chartoix) + 1).zero_(), requires_grad=False)
                for i in ix_to_oix:
    ...
    cycle_loss_func = nn.CrossEntropyLoss() if params['cycle_loss_type'] == 'ml' else nn.CosineEmbeddingLoss(
        margin=0.1) if params['cycle_loss_func'] == 'cosine' else nn.L1Loss()
    ...
    if params['tensorboard']:
        ...
        try:
            ...
            print
            'No previous experiment of same name found'
    ...
    for i in xrange(total_iters):
        ...
        if params['feature_matching'] or params['ml_update']:
            ...
            if params['feature_matching']:
                ...
                targ_mean_vec = eval_out_gt[1].mean(dim=0).detach() if params['weigh_feat_match'] == 0. else (
                            eval_out_gt[1] * feat_match_weight[:, None]).sum(dim=0).detach()
        ...
        for gi in xrange(iters_gen):
            # import ipdb; ipdb.set_trace()
            outs = adv_forward_pass(modelGen, modelEval, inps, lens, end_c=misc['char_to_ix'][misc['endc']],
                                    maxlen=params['max_seq_len'], auths=auths, cycle_compute=(
                            params['cycle_loss_type'] != None and params['cycle_loss_type'] != 'noncyc_enc'),
                                    cycle_limit_backward=params['cycle_loss_limitback'], append_symb=append_tensor,
                                    temp=params['gumbel_temp'],
            ...
            if params['cycle_loss_type'] == 'bow':
                ...
                cyc_targ = Variable(torch.cuda.FloatTensor(params['batch_size'],
                                                           params['vocabulary_size'] + 1).zero_().scatter_add_(1,
                                                                                                               targs.transpose(
                                                                                                                   0,
                                                                                                                   1).cuda(),
                                                                                                               torch.ones(
                ...
                if params['cycle_loss_func'] == 'cosine':
                    cyc_loss = params['cycle_loss_w'] * cycle_loss_func(rev_enc_orig_order, enc_inp_gt.detach(),
                                                                        ones[:rev_enc_orig_order.size(0)])
                else:
                    cyc_loss = params['cycle_loss_w'] * cycle_loss_func(rev_enc_orig_order, enc_inp_gt.detach())
                ...
                # Compute encodings for the generated reverse sample!
                # -----------------------------------------------------------------------------------------------------------
                gen_lens, char_outs, gen_samples, rev_gen_lens, rev_char_outs = outs[3], outs[4], outs[-3], outs[-2], \
                                                                                outs[-1]
            ...
            if params['language_loss']:
                ...
                gen_samples_srt = gen_samples.index_select(1, Variable(gen_lensort_idx, requires_grad=False))
                enc_inp = torch.cat([append_tensor, gen_samples_srt])
                n_steps = enc_inp.size(0);
                b_sz = enc_inp.size(1);
                targ_aid = 1 - c_aid
                langModelInp = enc_inp.view(n_steps * b_sz, -1).mm(mapVocabToLangModel[targ_aid]).view(n_steps, b_sz,
                                                                                                       -1)
                langProb, _ = langModel[targ_aid].forward_mltrain(langModelInp, len_sorted.tolist(), langModelInp,
                                                                  len_sorted.tolist(), adv_targ=True)
                langModelTarg = pack_padded_sequence(Variable(langModelInp.data[1:, :, :].max(dim=-1)[1]),
                                                     len_sorted.tolist())
                lang_loss = params['language_loss'] * ml_criterion(
                    pack_padded_sequence(langProb, len_sorted.tolist())[0], langModelTarg[0])
                if lang_loss.data[0] > 20:
                    print
                    'Limiting loss', lang_loss
                    lang_loss = 0.
            else:
                lang_loss = 0.
            ...
            if 0:
                ...
                if not params['maximize_entropy']:
                    if params['weigh_difficult'] > 0.:
                        loss_aid = FN.binary_cross_entropy_with_logits(gen_aid_out, ones[:gen_aid_out.size(0)],
                                                                       sample_weight, size_average=True)
                    else:
                    ...
                    b2 = 0.7
                    pdet = p.detach()
                    loss_aid = (-torch.log(p) * (pdet < b1).float() + float(-np.log(b1)) * (
                                pdet >= b1).float() + slp * (p - b2) * (pdet >= b2).float()).mean()
                lossGen = 5 * loss_aid
            elif params['wasserstien_loss']:
                targ_aid = 1 - c_aid
                gen_aid_out = outs[0][:, targ_aid]
            ...
            accum_err_eval[targ_aid] += (
                (gen_aid_out.data > 0.).float().mean())  # + (eval_out_gt[0][:,targ_aid].data <= 0.).float().mean())/2.
        ...
        avg_feat_loss += feature_match_loss.data.cpu().numpy()[0] if type(
            feature_match_loss) != float else feature_match_loss
        # ===========================================================================
        ...
        if i % 10 == 9:
            err_a1, err_a2 = accum_err_eval[0] / (accum_count_gen[0] + 1e-6), accum_err_eval[1] / (
                        accum_count_gen[1] + 1e-6)
        ...
        if i % params['log_interval'] == (params['log_interval'] - 1):
            ...
            lossEv_generic = avgL_generic / (accum_count_eval.sum() + 1e-5) + (
                        accum_count_eval.sum() == 0) * lossEv_generic
            ...
            print('| epoch {:2.2f} | {:5d}/{:5d} batches | lr {:02.2e} | ms/it {:5.2f} | t {:2.2f} | '
                ...
                elapsed * 1000 / args.log_interval, modelGen.temp.data.mean(), lossG, lossG_gan, lossGcyc, lossGfeat,
                lossEv_tot, 100. * err_a1, 100. * err_a2,
                lossEv_const))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--authstring', dest='authstring', type=str, default='author',
                        help='which label to use as author')
    parser.add_argument('--filterauths', dest='filterauths', nargs='+', type=str, default=[],
                        help='which author classes to keep')
    parser.add_argument('--filtertype', dest='filtertype', type=str, default='keep',
                        help='which author classes to keep')

    parser.add_argument('--use_unk', dest='use_unk', type=int, default=1, help='Use UNK for out of vocabulary words')
    parser.add_argument('--sample_by_len', dest='sample_by_len', type=int, default=1,
                        help='Use UNK for out of vocabulary words')
    parser.add_argument('--uniform_len_sample', dest='uniform_len_sample', type=int, default=0,
                        help='uniform_len_sample')
