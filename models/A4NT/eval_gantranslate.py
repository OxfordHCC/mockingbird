...
def adv_forward_pass(modelGen, modelEval, inps, lens, end_c=0, maxlen=100, auths=None,
    ...
    gen_samples, gen_lens, char_outs = modelGen.forward_advers_gen(inps, lens, end_c=end_c, n_max=maxlen, auths=auths,
                                                                   n_samples=n_samples, soft_samples=(n_samples > 1))
    ...
    if cycle_compute:
        ...
        _, rev_gen_lens, rev_char_outs = modelGen.forward_advers_gen(reverse_inp, len_sorted.tolist(), end_c=end_c,
...
def main(params):
    ...
    result = {'docs': [], 'misc': {'auth_to_ix': auth_to_ix, 'ix_to_auth': ix_to_auth}, 'cp_params': cp_params,
              'params': params}
    id_to_ix = {}
    ...
    for i, b_data in tqdm(enumerate(dp.iter_sentences_bylen(split=params['split'], atoms=cp_params.get('atoms', 'word'),
                                                            batch_size=params['batch_size'], auths=auth_to_ix.keys()))):
        ...
        for b in xrange(inps.size()[1]):
            ...
            for si in xrange(n_samp):
                ...
                trans_text = jc.join(
                    [ix_to_char[c.cpu()[si, b]] for c in gen_char[:gen_lens[si, b]] if c.cpu()[si, b] in ix_to_char])
                samples.append({'sent': inp_text, 'score': eval_out_gt[0][b].data.cpu().tolist(), 'trans': trans_text,
                                'trans_score': gen_scores[si, b].cpu().tolist(), 'sid': b_data[0][b]['sid']})
        ...
        if params['print']:
            ...
            '--------------------------------------------'
            print
            'Author: %s' % (b_data[0][0]['author'])
            print
            'Inp text %s: %s (%.2f)' % (
            ix_to_auth[auths[0]], jc.join([ix_to_char[c[0]] for c in inps[1:]]), real_aid_out[0])
            print
            'Out text %s: %s (%.2f)' % (
            ix_to_auth[auths_inp[0]], jc.join([ix_to_char[c.cpu()[0]] for c in outs[1] if c.cpu()[0] in ix_to_char]),
            gen_aid_out[0])
            if params['show_rev']:
                print
                'Rev text %s: ' % (ix_to_auth[auths[0]]) + '%s' % (
                    jc.join([ix_to_char[c.cpu()[0]] for c in outs[-2] if c.cpu()[0] in ix_to_char]))
        # else:
    ...
    err_real_a1, err_real_a2 = accum_err_real[0] / (1e-5 + accum_count_gen[0]), accum_err_real[1] / (
                1e-5 + accum_count_gen[1])
    print
    '--------------------------------------------'
    print
    'Efficiency in fooling discriminator'
    print
    '--------------------------------------------'
    ...
    print
    'Mean difference : translation to %s = %.2f , translation to %s = %.2f ' % (
    ix_to_auth[0], diff_arr0.mean(), ix_to_auth[1], diff_arr1.mean())
    print
    'Difference > 0  : translation to %s = %.2f%%, translation to %s = %.2f%% ' % (
    ix_to_auth[0], 100. * (diff_arr0 > 0).sum() / (1e-5 + diff_arr0.shape[0]), ix_to_auth[1],
    100. * (diff_arr1 > 0).sum() / (1e-5 + diff_arr1.shape[0]))
    print
    'Difference < 0  : translation to %s = %.2f%%, translation to %s = %.2f%% ' % (
    ix_to_auth[0], 100. * (diff_arr0 < 0).sum() / (1e-5 + diff_arr0.shape[0]), ix_to_auth[1],
    100. * (diff_arr1 < 0).sum() / (1e-5 + diff_arr1.shape[0]))
    ...
    '\n--------------------------------------------'
    print
    'Consistencey with the input text'
    print
    '--------------------------------------------'
    print
    'Generated text A0- Precision = %.2f, Recall = %.2f' % (
    accum_prec_forward[0] / accum_count_gen[0], accum_recall_forward[0] / accum_count_gen[0])
    print
    'Generated text A1- Precision = %.2f, Recall = %.2f' % (
    accum_prec_forward[1] / accum_count_gen[1], accum_recall_forward[1] / accum_count_gen[1])
    if params['show_rev']:
        print
        '\n'
        print
        'Reconstr  text A0- Precision = %.2f, Recall = %.2f' % (
        accum_prec_rev[0] / accum_count_gen[0], accum_recall_rev[0] / accum_count_gen[0])
        print
        'Reconstr  text A1- Precision = %.2f, Recall = %.2f' % (
        accum_prec_rev[1] / accum_count_gen[1], accum_recall_rev[1] / accum_count_gen[1])
    ...
    '-------------'
    print
    'Doc accuracy is %s : %.2f , %s : %.2f' % (
    ix_to_auth[0], (doc_accuracy[0] / doc_count[0]), ix_to_auth[1], (doc_accuracy[1] / doc_count[1]))
    fp = doc_count[1] - doc_accuracy[1]
    recall = doc_accuracy[0] / doc_count[0]
    precision = doc_accuracy[0] / (doc_accuracy[0] + fp)
    f1score = 2. * (precision * recall) / (precision + recall)
    print
    'Precision is %.2f : Recall is %.2f , F1-score is %.2f' % (precision, recall, f1score)
    print
    '\nTranslated data'
    print
    '-----------------'
    print
    'Doc accuracy is %s : %.2f , %s : %.2f' % (
    ix_to_auth[0], (doc_accuracy_trans[0] / doc_count[0]), ix_to_auth[1], (doc_accuracy_trans[1] / doc_count[1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', '--genmodel', dest='genmodel', type=str, default=None,
                        help='generator/GAN checkpoint filename')
