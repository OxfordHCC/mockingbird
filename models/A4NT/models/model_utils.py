...
def packed_mean(packed, dim=0):
    ...
    lens = unp[1]
    sizes = unp_data.size()
    mean = torch.sum(unp_data, dim=dim, keepdim=False) / Variable(
        torch.FloatTensor(lens).view(-1, 1).expand(sizes[1], sizes[2]), requires_grad=False).cuda()
    return mean


def packed_add(p1, p2):
    # Assuming sequence lengths are fixed
    unp1 = pad_packed_sequence(p1);
    unp2 = pad_packed_sequence(p2)
    unp1_data = unp1[0];
    unp2_data = unp2[0];
    lens = unp1[1]
