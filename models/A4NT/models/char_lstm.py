...
class CharLstm(nn.Module):
    def __init__(self, params):
        ...
        if self.en_residual:
            ...
            self.rec_layers = nn.ModuleList(
                [nn.LSTM(inp_sizes[i], self.hidden_size, 1, bidirectional=self.bidir) for i in
                 range(self.num_rec_layers)])
        else:
        ...
        if params.get('mode', 'generative') == 'classifier':
            decoder_size = self.hidden_size * (1 + ((self.max_pool_rnn == 1) | (self.max_pool_rnn == 3))) * (
                        1 + self.bidir)
            if params.get('decoder_mlp', 0):
                ...
                self.decoder_cnn = nn.ModuleList(
                    [nn.Conv1d(self.hidden_size, self.cnn_nfilt, K, padding=K) for K in self.cnn_ks])
    ...
    def init_weights(self):
        ...
        if self.compression_layer:
            ...
            qn = torch.norm(self.compression_W.weight.data, p=1, dim=1).view(-1, 1).expand_as(
                self.compression_W.weight.data)
        ...
        if self.en_residual:
            for i in range(self.num_rec_layers):
                ...
                if self.bidir:
                    self.rec_layers[i].bias_ih_l0_reverse.data.index_fill_(0, torch.arange(h_sz + 1, h_sz * 2).long(),
                                                                           2.)
                    self.rec_layers[i].bias_hh_l0_reverse.data.index_fill_(0, torch.arange(h_sz + 1, h_sz * 2).long(),
                                                                           2.)
        else:
            for i in range(self.num_rec_layers):
                getattr(self.rec_layers, 'bias_ih_l' + str(i)).data.index_fill_(0,
                                                                                torch.arange(h_sz + 1, h_sz * 2).long(),
                                                                                2.)
                getattr(self.rec_layers, 'bias_hh_l' + str(i)).data.index_fill_(0,
                                                                                torch.arange(h_sz + 1, h_sz * 2).long(),
                                                                                2.)
                if self.bidir:
                    getattr(self.rec_layers, 'bias_ih_l' + str(i) + '_reverse').data.index_fill_(0,
                                                                                                 torch.arange(h_sz + 1,
                                                                                                              h_sz * 2).long(),
                                                                                                 2.)
                    getattr(self.rec_layers, 'bias_hh_l' + str(i) + '_reverse').data.index_fill_(0,
                                                                                                 torch.arange(h_sz + 1,
                                                                                                              h_sz * 2).long(),
                                                                                                 2.)
    ...
    def forward_eval(self, x, h_prev, compute_softmax=True):
        ...
        if compute_softmax:
            prob_out = self.softmax(dec_out.contiguous().view(-1, self.output_size)).view(n_steps, n_auth,
                                                                                          self.output_size)
        else:
            prob_out = dec_out
