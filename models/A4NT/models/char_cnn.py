...
class CharCNN(nn.Module):
    def __init__(self, params):
        ...
        self.decoder_cnn = nn.ModuleList(
            [nn.ModuleList([nn.Conv1d(cnn_inp_sizes[i], self.cnn_nfilt, K, padding=K // 2) for K in self.cnn_ks]) for i
    ...
    def forward_classify(self, x, h_prev=None, compute_softmax=False, predict_mode=False, adv_inp=False, lens=None):
        ...
        for i in xrange(self.n_layers):
            ...
            cnn_out_padded.append([FN.pad(cnn_out[-1][j].unsqueeze(3), (0, 0, (max_sz - cnn_out[-1][j].size(2)) // 2,
                                                                        int(((max_sz - cnn_out[-1][j].size(
                                                                            2)) / 2. + 0.5) // 1))).squeeze(3) for j in
                                   xrange(len(cnn_out[-1]))])
            cnn_inp.append(self.dec_drop(torch.cat(cnn_out_padded[-1], dim=1)))
            if self.decoder_cnn_residual and i > 0 and (i % 2) == 0:
                cnn_inp[-1] = cnn_inp[-1] + FN.pad(cnn_inp[-3].unsqueeze(3),
                                                   (0, 0, (max_sz - cnn_inp[-3].size(2)), 0)).squeeze(3)
