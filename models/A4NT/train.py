...
def main(params):
    ...
    if params['resume'] == None:
        if params['atoms'] == 'char':
            char_to_ix, ix_to_char = dp.create_char_vocab(params['vocab_threshold'])
        else:
            char_to_ix, ix_to_char = dp.create_word_vocab(params['vocab_threshold'])
    ...
    for i in xrange(total_iters):
        ...
        if params['randomize_batches']:
            ...
            batch = dp.get_sentence_batch(params['batch_size'], split='train', aid=c_aid, atoms=params['atoms'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--balance_loss', dest='balance_loss', nargs='+', type=float, default=[],
                        help='weigths for each class in loss function')

    # mode
    parser.add_argument('--modeltype', dest='modeltype', type=str, default='lstm',
                        help='Use LSTM based or CNN based model')
    ...
    parser.add_argument('--compression_layer', dest='compression_layer', type=int, default=0,
                        help='Use a compression layer')

    parser.add_argument('--decoder_mlp', dest='decoder_mlp', type=int, default=0,
                        help='Use Mlp layer to do the classification')
    parser.add_argument('--decoder_cnn', dest='decoder_cnn', type=int, default=0,
                        help='Use CNN layer to do the classification')
    parser.add_argument('--decoder_cnn_residual', dest='decoder_cnn_residual', type=int, default=0,
                        help='Use CNN layer to do the classification')
    parser.add_argument('--decoder_cnn_nlayers', dest='decoder_cnn_nlayers', type=int, default=1,
                        help='Use CNN layer to do the classification')
    parser.add_argument('--decoder_cnn_nfilt', dest='decoder_cnn_nfilt', type=int, default=100,
                        help='Use Mlp layer to do the classification')
    parser.add_argument('--decoder_cnn_ks', dest='decoder_cnn_ks', nargs='+', type=int, default=[2, 3, 4],
                        help='Use Mlp layer to do the classification')
    ...
    parser.add_argument('--decay_rate', dest='decay_rate', type=float, default=0.99,
    ...
    parser.add_argument('--drop_prob_encoder', dest='drop_prob_encoder', type=float, default=0.25,
                        help='what dropout to apply right after the encoder to an RNN/LSTM')
    parser.add_argument('--drop_prob_decoder', dest='drop_prob_decoder', type=float, default=0.25,
    ...
    # LSTM parameters
    parser.add_argument('--en_residual_conn', dest='en_residual_conn', type=int, default=0,
                        help='depth of hidden layer in generator RNNs')
    parser.add_argument('--hidden_depth', dest='hidden_depth', type=int, default=1,
                        help='depth of hidden layer in generator RNNs')
