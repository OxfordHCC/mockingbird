...
def main(params):
    ...
    for i, doc in enumerate(resInp['docs']):
        ...
        for j, st in enumerate(doc['sents']):
            if type(st) == list:
                for sent in st:
                    resCandsImgid[imgid + '+' + str(j)].append(
                        {'image_id': imgid, 'caption': ' '.join(sent['trans'].split()[:-1]), 'id': icnt})
                    resGtImgid[imgid + '+' + str(j)].append(
                        {'image_id': imgid, 'caption': ' '.join(sent['sent'].split()[:-1]), 'id': icnt})
                    icnt += 1
            else:
                resCandsImgid[imgid + '+' + str(j)].append(
                    {'image_id': imgid, 'caption': ' '.join(st['trans'].split()[:-1]), 'id': icnt})
                resGtImgid[imgid + '+' + str(j)].append(
                    {'image_id': imgid, 'caption': ' '.join(st['sent'].split()[:-1]), 'id': icnt})
                icnt += 1
    ...
    for i in xrange(maxlen):
        ...
        for k in resGtImgid.keys():
            if i < len(resCandsImgid[k]):
                ...
                gts[k] = resGtImgid[k]
        print
        'Now in %d, Lengths %d' % (i, len(gts))
        t0 = time.time()
        score, scores = scorer.compute_score(gts, res)
        dt = time.time() - t0
        print
        'Done %d in %.3fs, score = %.3f' % (i, dt, score)
        icnt = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', dest='refdata', type=str,
                        default='/triton/ics/project/imagedb/picsom/databases/COCO/download/annotations/captions_val2014.json',
                        help='file with reference captions')
