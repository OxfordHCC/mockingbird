...
def eval_model(dp, model, params, char_to_ix, auth_to_ix, split='val', max_docs=-1, dump_scores=False):
    ...
    print('Corr is %.2f | Max is %.2f | Min is %.2f' % (
    mean_corr_prob / n_docs, mean_max_prob / n_docs, mean_min_prob / n_docs))
    print('-----------------------------------')
    ...
    adjusted_scores = ((np.argsort(all_docs[:, np.concatenate([all_auths, neg_auths])], axis=0) == np.concatenate(
        [np.arange(n_docs), np.arange(n_docs)])).argmax(axis=0) + 1) / n_docs
    print('Accuracy per adjusted scores %.3f' % (
                100. * ((adjusted_scores[:int(n_docs)] >= 0.5).sum() + (adjusted_scores[int(n_docs):] < 0.5).sum()) / (
                    2. * n_docs)))
    auc = roc_auc_score(np.concatenate([np.ones(int(n_docs), dtype=int), np.zeros(int(n_docs), dtype=int)]),
                        adjusted_scores)
    print('AUC is  %.2f' % (auc))
...
def eval_classify(dp, model, params, char_to_ix, auth_to_ix, split='val', max_docs=-1,
    ...
    for i, b_data in tqdm(
            enumerate(dp.iter_sentences(split=split, atoms=params.get('atoms', 'char'), batch_size=b_sz))):
        # for i, b_data in tqdm(enumerate(dp.iter_single_doc(split=split, max_docs=max_docs))):
        ...
        correct_textblock_topk += (
                    np.where(scores.argsort(axis=1)[:, ::-1] == auths.numpy()[:, None])[1] <= params.get('topk',
                                                                                                         5)).sum()
        n_blks = n_blks + c_sz
        ...
        if done:
            ...
            mean_rank = mean_rank + np.where(current_doc_score.argsort()[::-1] == auths[0])[0][0]
            correct_topk += (
                        np.where(current_doc_score.argsort()[::-1] == auths[0])[0][0] <= params.get('topk', 5)).sum()
            mean_corr_prob = mean_corr_prob + current_doc_score[auths[0]]
            mean_max_prob = mean_max_prob + current_doc_score.max()
            mean_min_prob = mean_min_prob + current_doc_score.min()
            all_doc_scores.append(current_doc_score[None, :])
    ...
    if dump_scores:
        pickle.dump({'scores': all_window_scores, 'authors': all_auths},
                    open('window_scores_' + 'blog-age_teenVsRes_dep2res_0.5drop_uniformlen_0.68' + split + '.p', 'w'))
    ...
    for i in range(n_auths):
        print('Block level accuracy of class %s is %.2f.' % (
            ix_to_auth[i], 100. * (correct_textblock_perclass[i] / block_perclass[i])))
    ...
    if 0:  # model.num_output_layers >2:
        ...
        print('Accuracy per adjusted scores %.3f' % (100. * (
                    (adjusted_scores[:int(n_docs)] >= 0.5).sum() + (adjusted_scores[int(n_docs):] < 0.5).sum()) / (
                                                                 2. * n_docs)))
        auc = roc_auc_score(np.concatenate([np.ones(int(n_docs), dtype=int), np.zeros(int(n_docs), dtype=int)]),
                            adjusted_scores)
        print('AUC is  %.2f' % (auc))
