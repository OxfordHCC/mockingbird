...
from utils.preproc_dataset import preproc_dataset
...
class DataProvider:
    ...
    def __init__(self, params):
        ...
        self.athstr = 'author' if params.get('authstring', None) is None else params['authstring']
        ...
        if len(params.get('filterauths', [])) > 0:
            if params.get('filtertype', 'keep') == 'keep':
                ...
                groupidces = list(map(int, params['filterauths']))
        ...
        # relevant, they might need to be forgotten, esp. when network in changing
        ...
        if dataset == 'blogdata':
            ...
            raise ValueError('ERROR: Don\'t know how to do len splitting for this dataset')
    ...
    def set_hid_cache(self, idces, hid_state):
        ...
        # This allows setting multiple indices to same vector
        n_hid = hid_state[0].data.size()[1]
        for i, cid in enumerate(idces):
            self.hid_cache[cid] = [torch.index_select(hd.data, 1, torch.LongTensor([min(i, n_hid - 1)]).cuda()) for hd
                                   in
                                   hid_state]
        return

    def get_hid_cache(self, idces, hid_state):
        for i in range(len(hid_state)):
    ...
    def iter_single_doc(self, split='train', max_docs=-1):
        # Since this is a standalone iteration usually run to completion
        # we want to have a separate temporary doc pointers here.
            ...
            for aid in auths:
                for idx in self.lenMap[split][l]:
                    if self.data['docs'][idx[0]][self.athstr] == aid:
                        ...
                        batch.append(
                            {'in': inp, 'targ': targ, 'author': self.data['docs'][idx[0]][self.athstr], 'id': idx[0],
                             'sid': idx[1]})
    ...
    def iter_sentences(self, split='train', batch_size=100, atoms='char'):
        ...
        for i, cid in enumerate(self.splits[split]):
            for j in range(self.get_num_sent_doc(cid, atoms=atoms)):
                inp, targ = sent_func[atoms](cid, sidx=j)
                batch.append(
                    {'in': inp, 'targ': targ, 'author': self.data['docs'][cid][self.athstr], 'id': cid, 'sid': j})
    ...
    def get_num_sent_doc(self, cid, atoms='char'):
        sents = [st for st in self.data['docs'][cid]['text'].split('.') if len(st) > 0] if atoms == 'char' else \
            self.data['docs'][cid]['tokens']
        return len(sents)

    def get_doc_batch(self, split='train'):
        act_ids = np.where(self.cur_batch >= 0)[0]
        dead_ids = np.where(self.cur_batch < 0)[0]
        self.cur_batch[dead_ids] = np.random.choice(list(set(self.splits[split]) - set(act_ids.tolist())),
                                                    len(dead_ids), replace=False)
        batch = []
    ...
    def get_rand_sentence_tokenized(self, cid, sidx=None):
        ...
        sidx = np.random.randint(0, len(sents), 1) if sidx is None else sidx
    ...
    def get_sentence_batch(self, batch_size, split='train', atoms='char', aid=None, sample_by_len=False):
        allids = self.lenMap[split][self.get_rand_len()] if sample_by_len else self.splits[split]
        if aid:
    ...
    def prepare_data(self, batch, char_to_ix, auth_to_ix, leakage=0., maxlen=None):
        ...
        return torch.from_numpy(inp_seqs_arr), torch.from_numpy(targ_seqs_arr), torch.from_numpy(auths_arr), \
               lens_arr.tolist()

    def create_author_idx(self):
        author_idx = {}
        n_authors = 0
    ...
    def create_char_vocab(self, threshold=5):
        minivocab = {}
        ixtochar = {}
        vocab = Counter()
        for i in self.splits['train']:
    ...
    def create_word_vocab(self, threshold=5):
        minivocab = {}
        ixtochar = {}
        vocab = Counter()
        for i in self.splits['train']:
