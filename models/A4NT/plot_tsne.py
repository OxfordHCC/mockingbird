...
def onpick3(event):
    ...
    print('')
    for idx in ind:
        print('onpick3 scatter: %5d - Age %2d, Gender %6s:  %s' % (
        idx, uniqAge[(idx // n_st_perclass)], inv_gender_map[all_gender[sent_smallset_idx[idx]]], sent_subset[idx]))


fig = figure()
ax1 = fig.add_subplot(111)
legendList = [str(ag) for ag in uniqAge]
sc = ax1.scatter(tsnEmbSubset[:, 0], tsnEmbSubset[:, 1], c=all_age[sent_smallset_idx], picker=True, cmap=cmap,
                 alpha=0.6)
...
sc = ax2.scatter(tsnEmbSubset[:, 0], tsnEmbSubset[:, 1], c=all_gender[sent_smallset_idx], picker=True, cmap=cmap,
                 alpha=0.6)
