...
def main(params):
    ...
    print
    'Total Entries = %d, Mean Score = %.2f Std Score = %.2f ' % (totalCount, mean_score, std_score)
    print
    '               Mean Mean Score = %.2f Mean Std  = %.2f ' % (mean_mean_score, mean_std)
    print
    'Final Counts are :'
    # for k in resDict:
    ...
    if params['skipped_data']:
        ...
        '--------------------------------------------'
        print
        'Total Skip Entries = %d, Skip majority count = %d ' % (tot_skipped, tot_skip_wins)
        print
        'Total Bad Skip = %di' % (tot_bad_skips)
        print
        'Final Counts are :'
        for k in resDict:
            print
            '%s = %.2f, %d' % (
            k, 100. * float(skip_resDict[k] + resDict[k]) / (totalCount + tot_skipped), skip_resDict[k])

        print
        'Pecentage of ties = %.2f, total = %d' % (
        100. * (1. - float(tot_skip_wins + majority_votes) / (totalCount + tot_skipped)),
        totalCount + tot_skipped - tot_skip_wins - majority_votes)
        print
        skip_resDict_combos
        ...
        print
        'Total Entries = %d, Mean Score = %.2f Std Score = %.2f ' % (totalCount + tot_skipped, mean_score, std_score)
        print
        '               Mean Mean Score = %.2f Mean Std  = %.2f ' % (mean_mean_score, mean_std)
        print
        'Final Counts are :'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--num_models', dest='num_models', type=int, default=4,
                        help='how many characters to generate per string')
    parser.add_argument('--majority_vote', dest='majority_vote', type=int, default=1,
                        help='how many characters to generate per string')
    parser.add_argument('--include_skipped', dest='skipped_data', type=str, default=None,
                        help='how many characters to generate per string')
